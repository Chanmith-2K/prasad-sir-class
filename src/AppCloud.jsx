...
function normalizeStudent(id, student) {
  return {
    id,
...
function filterStudents(students, queryText, grade, range = "all") {
...
function buildDailyMessage(students) {
  ...
  return [
    ...
    "",
    "Slip photos are stored inside the cloud database. Use the app Slip Gallery to view them.",
  ].join(NEWLINE);
}
...
function runSelfTests() {
  ...
  console.assert(buildDailyMessage([]).includes("Total Paid Students"), "Daily message should build");
  console.assert(csv.includes("Name,Class,Date"), "CSV should export Name, Class, Date headers");
  console.assert(buildSlipHtmlReport([]).includes("Slip Report"), "HTML report should build");
...
export default function AppCloud() {
  ...
  const filteredStudents = useMemo(() => filterStudents(students, queryText, gradeFilter, dateRange), [students, queryText, gradeFilter, dateRange]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const rows = snapshot.docs.map((item) => normalizeStudent(item.id, item.data()));
        setStudents(rows);
        setLoading(false);
        setNotice("Cloud sync active. ඕනම phone/laptop එකකින් same data පේනවා.");
      },
      (error) => {
        console.error(error);
        setLoading(false);
        setNotice(`Firebase read error: ${error.message}`);
      }
    );
    return () => unsubscribe();
  }, []);
  ...
  async function handleSlipUpload(event) {
    ...
    if (!file.type.startsWith("image/")) {
      setNotice("Payment slip එක image file එකක් වෙන්න ඕන.");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setNotice("Image එක ලොකුයි. 5MB ට අඩු screenshot/photo එකක් upload කරන්න.");
      return;
    }
    try {
      setNotice("Slip image එක compress වෙනවා...");
      const dataUrl = await compressImageFile(file);
      if (dataUrl.length > 900000) {
        setNotice("Compress කළාටත් image එක ලොකුයි. Screenshot එක crop කරලා නැවත upload කරන්න.");
        return;
      }
      updateForm("slipImage", dataUrl);
      setNotice("Payment slip photo එක upload/compress වුණා.");
    } catch (error) {
      console.error(error);
  ...
  async function addStudent() {
    ...
    const newStudent = {
      name: form.name.trim(),
      grade: form.grade,
      month: form.month.trim() || "May 2026",
      amount,
      joined: form.joined || getTodayDate(),
      enrolledBy: form.enrolledBy.trim(),
      notes: form.notes.trim() || "Paid enrollment",
    ...
    try {
      ...
      setNotice("Paid student record එක Firebase cloud database එකට save වුණා.");
    } catch (error) {
      console.error(error);
      setNotice(`Firebase save error: ${error.message}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function clearAllRecords() {
    const ok = window.confirm("Cloud database එකේ student records සියල්ල delete කරන්නද?");
    if (!ok) return;
    setSaving(true);
    try {
      const snapshot = await getDocs(studentsCollection);
      await Promise.all(snapshot.docs.map((item) => deleteDoc(doc(db, "students", item.id))));
      setNotice("Cloud student records සියල්ල clear වුණා.");
    } catch (error) {
      console.error(error);
      setNotice(`Firebase delete error: ${error.message}`);
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 rounded-3xl bg-slate-950 p-6 text-white shadow-lg md:p-8">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm text-slate-200">☁️ Cloud Synced Student Enrollment Manager</div>
              <h1 className="text-3xl font-extrabold tracking-tight md:text-5xl">Prasad Sir Class</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300 md:text-base">Payment කරපු students විතරක් add කරන්න. Data Firebase cloud database එකට save වෙන නිසා phone/laptop දෙකෙන්ම same data පේනවා.</p>
            </div>
        ...
        <section className="mb-6 grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <div className="p-6">
              ...
              <p className="mt-1 text-sm text-slate-500">Student name + slip photo + payment details cloud එකට save කරන්න.</p>
              ...
              <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><div className="text-3xl">📤</div><p className="mt-2 text-sm font-medium">Payment Slip Upload</p><p className="mt-1 text-xs text-slate-500">Image එක auto-compress වෙනවා. Storage upgrade ඕන නැහැ.</p><input type="file" accept="image/*" onChange={handleSlipUpload} className="mt-4 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" />{form.slipImage ? <img src={form.slipImage} alt="Uploaded payment slip preview" className="mx-auto mt-4 max-h-48 rounded-2xl border border-slate-200 object-contain" /> : null}</div>
              {notice ? <p className="mt-4 rounded-xl bg-slate-100 px-4 py-3 text-sm text-slate-700">{notice}</p> : null}
              <div className="mt-4 grid gap-3 md:grid-cols-2"><Button onClick={addStudent} disabled={saving} className="w-full bg-slate-950 text-white hover:bg-slate-800">{saving ? "Saving..." : "Save Paid Student Record"}</Button><Button onClick={clearAllRecords} disabled={saving} className="w-full bg-red-50 text-red-700 hover:bg-red-100">Clear Cloud Records</Button></div>
